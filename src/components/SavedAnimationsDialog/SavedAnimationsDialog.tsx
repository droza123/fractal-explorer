...
export function SavedAnimationsDialog() {
  ...
  const handleSaveAnimation = async () => {
    if (!saveAsName.trim() || keyframes.length < 2) return;
    await saveAnimation(saveAsName.trim());
    setSaveAsName('');
  };
  ...
  const canSave = keyframes.length >= 2;
  const currentDuration = calculateTotalDuration(keyframes);
  ...
  return (
    ...
    >
      <div
        className="bg-gray-900 rounded-xl shadow-2xl border border-gray-700 w-[480px] max-h-[85vh] overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {/* Import message */}
          {importMessage && (
            <div className="bg-blue-900/30 text-blue-400 text-sm px-3 py-2 rounded-lg">
              {importMessage}
            </div>
          )}
          ...
          {canSave && (
            <div className="bg-gray-800/50 rounded-lg p-3 space-y-3">
              <div className="text-sm text-gray-400">
                Current: {keyframes.length} keyframes, {formatDuration(currentDuration)}
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleSaveAnimation()}
                  placeholder="Animation name..."
                  className="flex-1 bg-gray-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                <button
                  onClick={handleSaveAnimation}
                  disabled={!saveAsName.trim()}
                  className="px-4 py-2 bg-purple-600 hover:bg-purple-500 disabled:bg-gray-600 disabled:opacity-50 text-white text-sm rounded-lg transition-colors"
                >
                  Save
                </button>
              </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-800 hover:bg-gray-700 disabled:opacity-50 text-gray-300 text-sm rounded-lg transition-colors"
            >
          ...
          {savedAnimations.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-1">Add keyframes to create an animation</p>
            </div>
          ) : (
            <div className="space-y-2">
              {savedAnimations.map((animation) => (
                <div
                  key={animation.id}
                  className={`bg-gray-800/50 rounded-lg p-3 flex items-center gap-3 group hover:bg-gray-800 transition-colors ${
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {deleteConfirmId === animation.id ? (
                      ...
                      <>
                        <button
                          onClick={() => handleLoadAnimation(animation.id!)}
                          className="p-1.5 rounded hover:bg-gray-600 text-gray-400 hover:text-purple-400 transition-colors"
